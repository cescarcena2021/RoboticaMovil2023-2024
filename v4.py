...
while True:
    ...
    altura_franja = height // 3

    # Cortar la imagen en tres partes iguales
    parte1 = img[0:altura_franja, :]
    parte2 = img[altura_franja:2 * altura_franja, :]
    parte3 = img[2 * altura_franja:3 * altura_franja, :]

    # Guardar las partes en archivos separados
    cv2.imwrite("parte1.jpg", parte1)
    cv2.imwrite("parte2.jpg", parte2)
    cv2.imwrite("parte3.jpg", parte3)

    # image process to make the imgae more smooth so that we can obtain more information
    hsv1 = cv2.cvtColor(parte2, cv2.COLOR_BGR2HSV)
    blur1 = cv2.GaussianBlur(hsv1, (5,5), 0)

    hsv2 = cv2.cvtColor(parte3, cv2.COLOR_BGR2HSV)
    ...
    cv2.circle(parte2,(cx1,cy1),10,(100,0,100),-1)
    cv2.circle(parte3,(cx2,cy2),10,(100,0,100),-1)
    
    # position PID control
    error = -(cx2 - width/2)/300
